        ...
        antennas_frequencies.iter().for_each(|first_antenna_cords|{
            antennas_frequencies.iter().for_each(|second_antenna_cords|{
                if !(first_antenna_cords==second_antenna_cords){
                    let mut mult = 1usize;
                    loop {
                        let cords = calculate_antinode(first_antenna_cords, second_antenna_cords,mult);
...
fn calculate_antinode (first_antenna: &(usize,usize), second_antenna: &(usize,usize), mult: usize) -> (usize,usize) {
    let vec = (mult as isize * (second_antenna.0 as isize - first_antenna.0 as isize), mult as isize * (second_antenna.1 as isize - first_antenna.1 as isize));
